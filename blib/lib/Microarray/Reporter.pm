...
Performs QC on the spot level data. This uses a number of variables whose defaults are set by the L<Microarray|Microarray> module during initialisation of a Microarray object, and whose values can be changed using appropriate L<Microarray|Microarray> methods. The L<Microarray|Microarray> method L<Microarray/"should_reject_unique"|should_reject_unique()> causes the QC process to reject any reporter that contains only a single spot. It should be called on the microarray object before the L<Microarray/"set_reporter_data"|set_reporter_data()> method. 
